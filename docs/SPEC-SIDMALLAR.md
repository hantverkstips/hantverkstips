...
4. **Sidan.** `src/pages/rakna/[slug].astro`, `prerender = false`, `Astro.locals.sidtyp = 'verktyg'`, `Cache-Control` på alla svar, `bred={true}`, brödsmulor Hantverkstips / Räkna själv / {verktyget}. Sidan renderar formulärkomponenten, resultatet med det stora talet, en `<Faktaruta variant="kortsvar">` överst, H2 "Så räknar vi" med formeln i ord och tabellen där varje rad är märkt Källa eller Antagande med länk, lagd i `<Tabellyta kolumner={3} class="mt-4">` (2.17) så att raden "Dra i sidled för att se hela tabellen" står över den på mobil, och H2 "Läs vidare". Delbar länk i ett skrivskyddat fält, byggd av de tolkade värdena. Strukturerad data: bara brödsmulorna, som layouten skriver ut. Verktyget är inte en artikel.
...
Markup: H1 "Behöver din altan bygglov? Fyll i måtten och få svar", ingress på fem meningar, `<Faktaruta variant="kortsvar">` med svaret, sedan formuläret och bedömningen i två spalter på det linjerade papperet. Resultatspalten bär beskedet, talet och länkarna, och ingenting mer (ändrat 2026-09-19): det stora "talet" är svarsordet i `text-siffra` med `<Markering>` och resten av rubriken på samma baslinje, precis där enheten står på de andra kalkylatorerna, med två metarader under om höjden, avståndet till byggnaden, avståndet till tomtgränsen och taket. Under dem byggsanktionsavgiften, alltså etiketten, talet i `text-h1` och raden om grundbeloppet, tillägget och prisbasbeloppet, men inte stycket om vad avgiften innebär: nedsättningen till hälften och en fjärdedel, rättelsen före sammanträdet och lovet i efterhand står sist i "Så bedömer vi", eftersom det är förklaring och inte svar. Sedan en rad i `text-liten text-blyerts-2` som pekar vidare till avsnitten under verktyget, länken "Så bedömer vi" och den delbara adressen. Vid standardvärdena är spalten 441 tecken, mot 1 496 före ombyggnaden. Direkt under kalkylatorn står H2 "Därför blev svaret så" med reglerna som en lista där varje rad har en etikett (Bygglov, Inget lov, Osäkert, Grannens ja), texten och lagrummet under, och sist i avsnittet rådet att kommunen har sista ordet och raden om att reglerna gäller sedan 1 december 2025; vid "vet inte" på detaljplan står de två fallens listor under varsin rubrik som förut. Sedan H2 "Gör inte det här" med raderna som stycken, H2 "Bygglov altan, de tre måtten och vad som ändrades 1 december 2025" (399 ord) med länk till artikeln, H2 "Så bedömer vi" med åtta steg och regeltabellen på tretton rader, och H2 "Läs vidare" till `/altan/bygglov-altan/` och `/guider/altan/`.
...
```ts
export interface AltanIndata { langdM: number; breddM: number; trallbreddMm: 95 | 120 | 145; riktning: 'langsida' | 'kortsida'; ccMm: 400 | 600; regel: '45x120' | '45x145' | '45x170' | '45x195' | '45x220'; grund: 'plintar' | 'befintlig' }
...
export const MAX_SPANNVIDD_M: Record<Regeldimension, Record<Regelavstand, number>>;   // Lathunden sid. 30
export const PLINTAVSTAND_M: Record<Barlinedimension, Record<FriLangd, number>>;      // Lathunden sid. 24
export function tabelleradFriLangd(spannviddM): FriLangd;     // rundar uppåt till 2,4, 3,6 eller 4,8 m
export function barlinaFor(regel): Barlinedimension;          // ANTAGANDE: samma dimension som regeln
export function plintavstand(regel, spannviddM, ccMm): number;
...
**Konstanterna med källa.** Trall 28 mm på c 600 mm som mest, springa 5, 6 och 7 mm för brädor på 95, 120 och 145 mm, två skruv per korsning för brädor från 95 mm och trallskruv 4,2 × 55 mm (samtliga TräGuiden, Läggning av trall). Fall 1:100 (byggbeskrivningen Altan). Spännvidderna och plintavstånden kommer sedan 2026-09-19 ur Svenskt Trä, Lathunden 8:2021, sid. 30 respektive sid. 24, kolumnerna C24, avlästa av skribenten på `/altan/reglar-avstand-och-dimensioner/` och radade upp i `docs/briefer/underlag-reglar-avstand-2026-09-19.md`. Förpackningarna om 250 eller 1 000 är butikskontroll 2026-09-19 (Bauhaus, Byggmax, K-Bygg, Essve; ingen säljer 500). ANTAGANDE: att bärlinan har samma dimension som regeln, hur förhållandet mellan c-måttet och plintavståndet tillämpas, handelslängder 3,6 till 4,8 m och spill 10 procent.

**Rättelsen 2026-09-19.** Tre fel lämnade modulen samma dag som Lathunden hämtades. `MAX_SPANNVIDD_M` hade rätt tal men fel källa, och raderna för 45 × 170 och 45 × 195 mm stod som ANTAGANDE trots att Lathunden bekräftar dem på centimetern; de är nu avlästa och märkta Källa, och 45 × 120 och 45 × 220 mm tillkom. Det täta c-måttet var 450 mm, ett mått som inte finns i någon tabell, och är nu Lathundens c 400 mm; en delad länk med `cc=450` läses som c 400 mm, alltså tätare än läsaren skrev. Och `PLINTAVSTAND_M` var 2,5 m rakt av, vilket ingen 45-dimension når ens vid den kortaste fria längden, så verktyget gav för få plintar; den är nu Lathundens hela tabell, uppslagen på bärlinans dimension och reglarnas fria längd, med en fri längd mellan två kolumner läst på den längre.

**Regeldimensionen styr plintarna.** Reglarnas fria spännvidd mellan två bärlinor sätts av regelns höjd. Räcker dimensionen inte till altanens djup lägger kalkylatorn in en bärlinrad till, tills spännvidden ligger under taket, och varje rad är en rad plintar. Inom raden sätts plintarna med det avstånd Lathundens bärlinetabell tillåter, kapat till högst fyra gånger reglarnas c-mått. Standardaltanen på 4 × 3 m med 45 × 145 mm får därför tre bärlinrader och tolv plintar, fyra i varje rad, medan samma altan på 45 × 195 mm klarar sig med två rader och åtta plintar. Före rättelsen stod det nio respektive sex. Rådet om spännvidd säger vilken dimension som hade tagit bort en rad.
...
Testskriptet `scripts/test-kalkyl-altan.mjs` kör nio fall: standardaltanen, riktningen bytt, 145 mm trall, c 400 mm, 45 × 195 mm på lång spännvidd, 45 × 145 mm som inte räcker till 4 m djup, en liten altan på 2 × 2 m, en stor på 8 × 5 m och standardaltanen på befintlig grund. Varje fall låser numera också plintavståndet och plintarna per rad. Utöver dem testas Lathundens två tabeller cell för cell, avrundningen uppåt till närmaste tabellrad, att bärlinan antas vara regeln, att förhållandet till c-måttet kapar avståndet, konstanterna mot underlaget, handelslängden, förpackningsvalet, gränserna och `tolkaQuery` inklusive den gamla `cc=450`.
...
Markup: H1 "Vad håller i gipsväggen? Välj plugg efter vikten", ingress på fyra meningar med fraserna "plugg gipsvägg", "gipsplugg", "hänga tungt i gipsvägg" och "tv-fäste på gipsvägg", `<Faktaruta variant="kortsvar">` med svaret på fem korta meningar, sedan formuläret och resultatet i två spalter på det linjerade papperet. Resultatspalten bär beskedet, tabellen och länkarna, och ingenting mer (ändrat 2026-09-19): det stora "talet" är beskedets ord i `text-siffra` med `<Markering>`, med resten av beskedet på samma baslinje; under det lasten per punkt i `text-h1`, förklaringen, och infästningstabellen med källrad, raden om vad som utelämnats och varningen när ingen rad räcker. Tabellen är själva svaret på den här sidan och stannar därför i spalten, till skillnad från reglerna på de andra beslutsverktygen. Den ligger i `<Tabellyta kolumner={3} class="mt-2">` (2.17) som sidans antagandetabell, så att ledtexten och scrollbehållaren ser likadana ut på de två tabellerna; spalten är hela bredden på mobil, och raden gör samma nytta där. Sedan en rad i `text-liten text-blyerts-2` som pekar vidare till "Gör inte det här" under verktyget, länken "Så bedömer vi" och den delbara länken. Vid standardvärdena är spalten 1 282 tecken, mot 1 413 före ombyggnaden; den är längre än de andra verktygens spalter eftersom tabellen på elva rader är svaret. Direkt under kalkylatorn står H2 "Gör inte det här" med råden som stycken, och avsnittet renderas bara när det finns råd att visa. Sidan har inget "Därför blev svaret så": verktyget bär ingen regellista, utan tabellen visar vad varje infästning klarar. Sedan H2 "Plugg i gipsvägg, vad gipspluggen bär och var gränsen går" (483 ord) med H3 "Hänga tungt i gipsvägg utan att öppna väggen" och H3 "Tv-fäste på gipsvägg, armen ändrar räkningen", H2 "Så bedömer vi" med åtta steg och antagandetabellen på tjugoen rader, H2 "Läs vidare" till `/inomhus/skruva-i-gipsvagg/`, `/inomhus/hanga-tavla-gipsvagg/` och `/inomhus/gipsskruv/`, och tre frågor i `<Faq>`.
...
Sökfraserna "vad kostar dränering" (H1, första stycket och första frågan), "dränering kostnad per meter" (H1, H2 och resultatets rader), "dränera om huset kostnad" (H2 och första frågan), "hur länge håller en dränering" (H3 och andra frågan) och "avfuktare eller dränering" (H3 och tredje frågan) ligger i rubrikerna och i första stycket. `<title>` "Dränering, kostnad per meter och om du behöver gräva", 52 tecken, bytt 2026-09-20 efter körning 2: "dränering kostnad" har 590 sökningar mot 320 för "vad kostar dränering", så title leder med den större frasen och H1 behåller läsarens fråga. Slugen finns i `MED_FORMULAR`, och `<Kalkylator namn="dranering" />` är inbäddad i `/fukt/fukt-i-kallaren/` direkt efter kostnadstabellens räkneexempel.
...
#### 4.7.12 `src/pages/rakna/kallare.astro` (byggd 2026-09-19)

`export const prerender = false`, samma motivering och samma `Cache-Control` som de elva sidorna ovan. `Astro.locals.sidtyp = 'verktyg'`. `reklam={false}`: verktyget pekar inte ut någon produktegenskap, det ger en diagnos, så sidan har varken produktkort eller reklamband. Brödsmulor: Hantverkstips / Räkna själv (`/rakna/`) / Självbesiktning av källaren. `bred={true}`.

Det fjärde beslutsverktyget, och det första som är en hel artikel gjord till ett formulär. Sidan är guiden `/fukt/fukt-i-kallaren/` som verktyg: läsaren kryssar i vad hon ser, vad tejptestet gav och vad hygrometern visar, och får en diagnos som pekar vidare till tejptestet, avfuktarkalkylatorn, dräneringsverktyget eller en fuktutredning. Ingenting i verktyget står utanför guiden eller dess underlag; utvecklaren hämtade inga nya källor.

Reglerna ligger i `src/lib/kalkyl/kallare.ts`:

```ts
export interface KallareIndata { symptom: Symptom[]; tejptest: 'markfukt' | 'kondens' | 'torrt' | 'inte-gjort'; luftfuktighet: number | null; arstid: 'sommar' | 'vinter'; anvandning: 'kallare' | 'bebodd' }
export const STANDARD = { symptom: [], tejptest: 'inte-gjort', luftfuktighet: null, arstid: 'sommar', anvandning: 'kallare' };
export const GRANSER = { luftfuktighet: [1, 100] };
export const SYMPTOM: SymptomRad[];                  // sex rader, guidens symptomtabell
export function bedomKallare(i): KallareResultat;    // ok | ogiltig
export function tolkaQuery(q): { indata: KallareIndata; harIndata: boolean };
```

Query: `se` (flera värden, en per kryssruta), `tejp`, `rf`, `arstid` och `bruk`. `tolkaQuery` läser kryssrutorna med `getAll`, sorterar dem alltid i symptomtabellens ordning oavsett hur de står i adressen, och låter okända värden falla bort tyst. Hygrometern tar decimalkomma och mellanslag i talet; ett tomt fält betyder inte mätt och är inget fel, medan skräp i fältet ger ett fel på just det fältet. Resultatet bär diagnosen, rubriken och raden, nästa steg med sin länk, de tre flaggorna `visaAvfuktare`, `visaDranering` och `visaUtredning`, de ikryssade raderna med guidens tolkning, `overKritiskRf`, reglerna i ordning och `gorInteDetHar`.

**De fem diagnoserna** är markfukt, kondens, läckage, hög luftfuktighet utan känd orsak, och vet inte än. Den sista är standardvärdenas svar, och det är med flit: sajtens råd är att inte köpa något förrän plasten har svarat, och den som landar från Google ska mötas av det svaret först, inte av en diagnos verktyget hittat på åt henne.

**Prioriteten mellan indata är sidans enda egentliga beslut, och den är vår.** Ingen källa rangordnar symptom, tejptest och hygrometer mot varandra. Ordningen är: läckaget först oavsett resten, sedan tejptestet, sedan symptomen, sedan hygrometern. Läckaget går före allt eftersom det är den enda av de tre orsakerna som kan bli akut och den enda tejptestet aldrig ser, för plasten sitter på en vägg medan vattnet kommer vid en genomföring. Tejptestet går före symptomen eftersom det är en mätning och inte ett intryck, och symptomen går före hygrometern eftersom hygrometern bara säger att luften är fuktig, aldrig varifrån vattnet kommer. Pekar två symptom åt olika håll gissar verktyget inte utan svarar vet inte än, som bygglovsverktyget gör vid "vet inte" på detaljplan. Raden står märkt som antagande både i steglistan och i tabellen.

**Reglerna i guidens ordning.** Varje regel bär sitt steg, `titta`, `tejpa`, `mat` eller `atgarda`, och listan under verktyget sorteras på det: titta först, tejpa sedan, mät, åtgärda sist. Kryssrutorna ger en `titta`-rad var med tabellens troliga orsak och nästa steg på raden; tejptestets fyra lägen ger var sin `tejpa`-rad; hygrometern, årstiden och vad källaren används till ger `mat`-rader; och diagnosen ger `atgarda`-raderna med åtgärd, pris och källa. Årstiden och användningen ändrar aldrig diagnosen, de lägger till en rad, eftersom ingen statistik finns över hur ofta de tre orsakerna förekommer.

**Konstanterna med källa.** `KRITISK_RF` 75 (Boverket, BBR 6:52). `KAN_HALLA_AR` 50 och saltutfällningen som tecken på fukt (Villaägarna). `TEJPTEST_DYGN` 2, alltså undre kanten mellan Gör Det Självs två veckor och en målerifirmas ett till två dygn. Kapillärsugningen (Polygon). Mikrobiell lukt som indikation på fuktskada och fukt i ett bebott rum som möjlig olägenhet för hälsan (Folkhälsomyndigheten). `AVFUKTARE_KR` 5 948 (Proffsmagasinet), `FUKTUTREDNING_KR` 5 355 (Ocab), `DRANERING_KR_PER_LOPMETER` 3 000 och `VILLAGRUND_LOPMETER` 40 (Villaägarna och guidens räkneexempel), Anticimex tre tal om vattnet från taket, och radonets referensnivå (Strålsäkerhetsmyndigheten). Alltså samma siffror som kostnadstabellen i `/fukt/fukt-i-kallaren/`. ANTAGANDE: prioritetsordningen ovan, symptomtabellen som helhet, att torr plast betyder fel vägg, `HYGROSTAT_RF` 60, `HYGROMETER_DYGN` 7, kondenssäsongen juli till september, undre gränsen på hygrometerfältet och gränsen på tio grader mellan kondens och sorption.

**Torr plast är den rad ingen konkurrent har.** Gör Det Själv beskriver ett av tejptestets tre utfall och säger ingenting om det torra. Att torrt på båda sidor betyder fel vägg och inte frisk källare är vår läsning, den står i guiden, och den står som antagande i tabellen. Den är också en av de fyra raderna i "Gör inte det här".

Markup: H1 "Gå igenom källaren själv och få veta varifrån vattnet kommer", ingress på tre meningar, `<Faktaruta variant="kortsvar">` på fem korta meningar, sedan formuläret och svaret i två spalter på det linjerade papperet. Resultatspalten bär beskedet, nästa steg och länkarna, och ingenting mer: etiketten "Trolig orsak", diagnosens ord i `text-siffra` med `<Markering>` där det stora talet står på de andra kalkylatorerna, beskedet på en rad, sedan "Nästa steg" med en mening och en länk till rätt avsnitt i guiden, en rad i `text-liten text-blyerts-2` som pekar vidare till avsnitten under verktyget, länken "Så bedömer vi" och den delbara adressen. Vid standardvärdena är spalten 418 tecken efter chefredaktörens genomgång 2026-09-19, mot dräneringens 989; den här sidan har inget tal och ingen posttabell att visa.

Direkt under kalkylatorn står `<Verktygskort kalkylator="avfuktare" />` vid kondens och `<Verktygskort kalkylator="dranering" />` vid markfukt, aldrig båda samtidigt, eftersom diagnosen bara kan vara en. Vid läckage eller oklart pekar i stället `atgarda`-raden på en fuktutredning med Ocabs pris. Sedan H2 "Därför blev svaret så" med reglerna som en lista där varje rad har sitt steg som etikett (Titta, Tejpa, Mät, Åtgärda), texten och källan under, och H2 "Gör inte det här" med de fyra raderna som stycken. Sedan H2 "Tre orsaker till fukt i källaren, och rätt åtgärd för var och en" (id `tre-orsaker`, 340 ord) i fem stycken utan H3, H2 "Så bedömer vi" med sex steg och antagandetabellen på tjugo rader, H2 "Läs vidare" till `/fukt/fukt-i-kallaren/`, `/fukt/luftfuktighet-inomhus/` och `/rakna/dranering/`, och tre frågor i `<Faq>`.

Formuläret i `src/components/kalkyl/KallareForm.astro`: symptomtabellens sex rader som kryssrutor under legenden "Vad ser du i källaren?", tejptestets fyra lägen som radioknappar med en förklarande rad under varje och en hjälprad som länkar till guiden, hygrometern som eget textfält med enheten i procent, platshållaren "Inte mätt" och hjälpraden om Boverkets gräns, och sist årstiden och vad källaren används till som radioknappar. Fälten står i guidens ordning: titta, tejpa, mät, och först därefter de två frågorna om sammanhanget. Kryssrutorna får ingen trolig orsak i etiketten; att visa svaret bredvid rutan hade styrt vad läsaren kryssar i. Chefredaktören fastställde det 2026-09-19 och satte samtidigt gränsen mot tejptestet, vars radioknappar tvärtom säger rakt ut vad varje utfall betyder: plasten är en mätning som redan är gjord och har ett entydigt utfall, medan symptomen är bedömningar som en etikett skulle färga. Hjälpraden över kryssrutorna säger att tolkningen kommer i svaret, så att tystnaden inte läses som hemlighetsmakeri. Samma dag beslöts att ingressen inte får eka guidens första stycke: den namnger de tre orsakerna i egna ord i stället för att skriva "ett av tre ställen", och guiden förklarar medan verktyget dömer.

Sökfraserna "fukt i källaren åtgärd" (H2 om de tre vattnen och första frågan), "markfukt eller kondens" (andra frågan och tejptestets rader) och "besiktiga källaren själv" (tredje frågan och titeln) ligger i rubrikerna och i första stycket. `<title>` "Besiktiga källaren själv och hitta varifrån vattnet kommer", 58 tecken; med suffixet blir den över 60 och layouten utelämnar det. Frasen "fuktig källare" ströks ur H1 och title 2026-09-20 efter Keyword Planner-körning 2: den har ingen egen data och slås ihop med "fukt i källaren", som är guidens fras, så verktyget siktar på sitt eget namn och lever som länktillgång, precis som avfuktarkalkylatorn. Slugen finns i `MED_FORMULAR`, och `<Kalkylator namn="kallare" />` är inbäddad i `/fukt/fukt-i-kallaren/` direkt efter symptomtabellens förklarande stycke, alltså där läsaren just fått veta vad hon ska titta efter.

Registret: `pelare: ['fukt', 'grund']` och säsong juli till oktober. Frasen "fukt i källaren" toppar i september med 880 sökningar och bottnar i december till januari med 170 enligt `docs/SOKORDSANALYS.md`, och kondensen som driver toppen är ett sommarproblem.

Testskriptet `scripts/test-kalkyl-kallare.mjs` kör trettiotre fall. Guiden läses från disk och parsas i skriptet, så att en ändrad rad i symptomtabellen eller ett omskrivet tejptest bryter testet i stället för att tyst glida ifrån verktyget: tabellens sex rader jämförs cell för cell mot modulens `SYMPTOM`, och tejptestets tre utfall söks som text i artikeln. Utöver dem testas varje diagnos för sig, alla tre prioritetsreglerna, Boverkets gräns från båda hållen, årstidens och användningens rader, att reglerna kommer i guidens ordning och att var och en bär en källa, de fyra råden, gränserna på hygrometerfältet och `tolkaQuery` med flera kryssrutor, decimalkomma, tomt fält och skräp.

#### 4.7.13 `src/pages/rakna/rotavdrag.astro` (byggd 2026-09-20)

`export const prerender = false`, samma motivering och samma `Cache-Control` som de tolv sidorna ovan. `Astro.locals.sidtyp = 'verktyg'`. `reklam={false}`: räkningen pekar inte ut någon produkt, den räknar på en faktura läsaren redan har, så sidan har varken produktkort eller reklamband. Brödsmulor: Hantverkstips / Räkna själv (`/rakna/`) / Rotavdrag 2026. `bred={true}`.

Det största verktyget på sajten mätt i sökvolym. "rotavdrag 2026" är 18 100 i månaden och växer 236 procent, och hela klustret är 18 460 enligt `docs/SOKORDSANALYS.md` avsnitt 7.4. **Skatteverket rankar inte i topp fem på sin egen regel.** Topp fem är bank, hantverksförmedlare, bank, offertförmedlare och låneförmedlare, och ingen av dem daterar sina tal, räknar, eller nämner att det är betalningsdatumet som avgör procentsatsen. Två faktafel i fältet går att slå på direkt: ettan på "renovera kök kostnad" anger rot-taket till 75 000 kr, vilket är det gemensamma taket för rot och rut, och ettan på "byta tak kostnad" nämner inte rotavdrag alls.

Formeln ligger i `src/lib/kalkyl/rotavdrag.ts`:

```ts
export interface RotavdragIndata { arbetskostnadKr: number; materialkostnadKr: number; antalAgare: number; utnyttjatRotKr: number; utnyttjatRutKr: number; skattKr: number | null }
export const STANDARD = { arbetskostnadKr: 100000, materialkostnadKr: 50000, antalAgare: 1, utnyttjatRotKr: 0, utnyttjatRutKr: 0, skattKr: null };
export const GRANSER = { arbetskostnadKr: [0, 5000000], materialkostnadKr: [0, 5000000], antalAgare: [1, 4], utnyttjatRotKr: [0, 500000], utnyttjatRutKr: [0, 500000], skattKr: [0, 5000000] };
export const ARET = 2026;
export function avdragRakt(arbetskostnadKr): number;    // procentsatsen, innan taken
export function kronor(n): string;                      // "120 000"
export function raknaRotavdrag(i): RotavdragResultat;   // ok | ogiltig
export function tolkaQuery(q): { indata: RotavdragIndata; harIndata: boolean };
```

Query: `arbete`, `material`, `agare`, `rot`, `rut` och `skatt`. Decimalkomma, mellanslag i talet, ett efterhängande "kr" och typografiskt minus tolkas. Ett tomt talfält betyder noll utom i skattefältet, där tomt betyder inte angivet och inte är ett fel. Resultatet bär avdraget, avdraget innan taken, vad taken kapade, hela fakturan, vad du betalar, delen utan avdrag med sin andel, de två taken totalt, vad som är kvar av dem, vilket av de fyra taken som band svaret, beskedet med rubrik och rad, reglerna i ordning och `gorInteDetHar`.

**Sidan döps om varje december.** Årtalet står som `ARET` i formelmodulen och bärs av `VERKTYGSNAMN`, `titel` och H1. Rutinen står i kommentaren vid `ARET` och i rutthuvudet: i december byts talet, procentsatsen och takbeloppen läses om hos Skatteverket, och de fyra ställena ändras i samma commit. Adressen `/rakna/rotavdrag/` ändras aldrig, så inkommande länkar överlever årsskiftet och ingen omdirigering behövs.

**Räkningen i sex steg.** Avdraget rakt av, alltså procentsatsen på arbetskostnaden. Rot-taket gånger antalet ägare minus utnyttjat rot. Det gemensamma taket gånger antalet ägare minus utnyttjat rot och utnyttjat rut. Skatten minus det som redan är utnyttjat, när fältet är ifyllt. Avdraget blir det minsta av de fyra. Att betala är arbete plus material minus avdraget.

**Två fält för det utnyttjade, inte ett.** Det utnyttjade rotavdraget äter av både rot-taket och det gemensamma taket, medan det utnyttjade rutavdraget bara äter av det gemensamma. Ett enda fält hade inte kunnat skilja dem åt, och skillnaden är hela skälet till att det gemensamma taket finns. Det är också den rad ettan på "renovera kök kostnad" har fel om.

**Konstanterna med källa, allihop hos Skatteverket.** `ROT_PROCENT` 30 och takbeloppen 50 000 och 75 000 kr ur Så fungerar rotavdraget. `ROT_PROCENT_HOJT` 50 med datumen 12 maj och 31 december 2025 ur nyheten om den tillfälliga höjningen. `NYBYGGT_SPARRAR_AR` 5 och ansökningsfristen 31 januari ur samma regelsida. Maskinell utrustning, uttryckligen grävmaskiner och borraggregat, ur Ger arbetet rätt till rotavdrag. Skatterna avdraget räknas av mot ur frågesidan om hur mycket man måste tjäna. Alla lästa 20 september 2026, och alla länkade per rad i antagandetabellen på arton rader. ANTAGANDE: fyra rader, alltså standardvärdet en ägare, att det utnyttjade beloppet gäller alla ägare tillsammans, gränserna på fälten, och förenklingen i skattefältet.

**Räkneexemplet är vårt, inte Skatteverkets. Rättat 2026-09-20 efter granskningen.** Första versionen påstod på fem ställen att talen 10 000, 3 000 och 7 000 kr var Skatteverkets eget räkneexempel på fakturamodellen. De står inte där. Myndighetens enda räkneexempel av den sorten ligger på företagssidan och utgår från arbetskostnaden **exklusive** moms: 10 000 kr blir 12 500 kr med moms och ger 3 750 kr i avdrag. Vårt fält frågar efter beloppet med moms, så samma faktura ger samma svar; skillnaden är vilket av de två talen läsaren skriver in. `EXEMPEL_*` står nu som våra egna tal i underlagets andra tabell, `SKV_EXEMPEL_*` bär myndighetens, momsen förklaras i en egen rad i antagandetabellen, och testskriptet låser formeln åt båda hållen. En påhittad källhänvisning får aldrig stå kvar, inte ens när talen den bär är rätt räknade.

**Standardvärdet en ägare är det försiktiga, med flit.** Två ägare dubblar taket, och ett verktyg som antar två skulle visa ett avdrag den ensamstående läsaren aldrig får. Hela fältets fel är att avdraget ser större ut än det blir, så vårt standardvärde lutar åt andra hållet.

Markup: H1 "Rotavdrag 2026, vad du drar av och vad du faktiskt betalar", ingress på tre meningar, `<Faktaruta variant="kortsvar">` på fem korta meningar, sedan formuläret och svaret i två spalter på det linjerade papperet. Resultatspalten bär beskedet, talet och länkarna: beskedet som en rad i H3-stil med en mening under, sedan det stora talet, alltså avdraget i `text-siffra` med `<Markering>` och "kr av arbetskostnaden" på samma baslinje, en rad i `text-liten` om vad det är räknat på, fyra rader med vad du betalar, delen utan avdrag och vad som är kvar av de två taken, en rad som pekar vidare till avsnitten under verktyget, länken "Så räknar vi" och den delbara adressen. Vid standardvärdena är spalten 559 tecken, mot dräneringens 989. Direkt under kalkylatorn står H2 "Därför blev svaret så" med reglerna som en lista där varje rad har sitt slag som etikett (Arbetet, Procentsatsen, Datumet, Taket, Skatten), texten och en länkad källa under, och H2 "Gör inte det här" med de fem raderna som stycken, där den fjärde är femårsregeln. Sedan H2 "Rotavdraget har tre tak, och de flesta känner bara till ett" med H3 "Taket per person, och taket ni delar", H3 "Skatten är det tredje taket" och H3 "Datumet som avgör, och som ingen skriver ut", H2 "Så räknar vi" med sex steg och antagandetabellen på arton rader, H2 "Läs vidare" till `/rakna/dranering/`, `/grund/dranera-hus/`, `/golv/bygga-trappa/` och `/rakna/kvadratmeter/`, och tre frågor i `<Faq>`.

Formuläret i `src/components/kalkyl/RotavdragForm.astro`: fakturans två poster under legenden "Vad står på fakturan?" med arbetet först, eftersom det är det enda som ger avdrag, antalet ägare som fyra radioknappar med hjälpraden om att varje ägare har ett eget tak, och sist de tre fälten under legenden "Vad är redan utnyttjat i år?" med rot, rut och den frivilliga skatten, vars platshållare är "Vet inte". Materialfältets hjälprad säger var maskinhyran och resekostnaderna ska skrivas in, eftersom skissen visar tre fakturarader medan formuläret har två fält.

Sökfraserna "rotavdrag 2026" (title, H1, ingress och första frågan), "rotavdrag hur mycket" (första frågan) och "rotavdrag beräkna" (ingressen och "Så räknar vi") ligger i rubrikerna och i första stycket. `<title>` "Rotavdrag 2026, räkna ut avdraget och vad du betalar", 52 tecken. Slugen finns i `MED_FORMULAR`, så `<Kalkylator namn="rotavdrag" />` går att bädda in i varje kostnadsavsnitt i rotklustret när det skrivs.

Registret: `pelare: ['grund', 'golv', 'kok', 'el']`. Fyra, inte åtta: Christians beslut 2026-09-20. Rot gäller förvisso arbete i varje pelare där man anlitar någon, men ett verktyg som står i åtta hubbars grupp Räkna står ingenstans, och de fyra är de där notan oftast är stor nog att taket biter. Säsong november till januari, med toppen i december: det är dagen du betalar som avgör vilket års tak avdraget hamnar på, så den som vill nå upp till taket betalar före nyår.

Testskriptet `scripts/test-kalkyl-rotavdrag.mjs` kör tjugofyra fall. Talen jämförs mot underlaget `docs/briefer/underlag-kalkyl-rotavdrag-2026-09-20.md`, vars båda tabeller parsas i skriptet: den ena över de nio talen som kommer från Skatteverket, där varje rad dessutom måste bära en myndighetsadress, och den andra över de tre räknetal som är våra egna, där ingen rad får tillskrivas en källa. Ett ändrat tal på ett av de två ställena bryter testet. Utöver dem testas att formeln ger 3 750 kr på Skatteverkets eget exempel, vårt eget räkneexempel, femårsregeln, att dateringsregeln pekar på regelsidan och inte på nyheten, att maskinistens arbetade tid ger avdrag, att villkorsregeln har alla sex villkoren inklusive hyresrätten, materialets andel, rot-taket med en och två ägare, det gemensamma taket när rut ätit av det, skillnaden mellan utnyttjat rot och utnyttjat rut, ett fullt utnyttjat tak, skatten som gräns, det tomma skattefältet, att varje regel bär en länkad Skatteverksadress, att båda datumen och båda procentsatserna står i svaret, att grävmaskiner och borraggregat nämns vid namn, gränserna på varje fält, `tolkaQuery` med decimalkomma och skräp, och tusentalsavgränsaren.

#### 4.7.14 `src/pages/rakna/trappa.astro` (byggd 2026-09-20)

`export const prerender = false`, samma motivering och samma `Cache-Control` som de tretton sidorna ovan. `Astro.locals.sidtyp = 'verktyg'`. `reklam={false}`: räkningen pekar inte ut någon produktegenskap, så sidan har varken produktkort eller reklamband. Brödsmulor: Hantverkstips / Räkna själv (`/rakna/`) / Trappräknare. `bred={true}`.

Verktyget siktar inte på egen trafik. "trappa steghöjd" är 50 i månaden, men vinnbarheten är 5 och artikeln verktyget bor i, `/golv/bygga-trappa/`, är 880 med en tom kategorisida som etta. Verktyget är alltså en länktillgång som avfuktarkalkylatorn och källarbesiktningen, och det bär sitt eget namn i titeln. Luckan i fältet är entydig enligt `docs/SOKORDSANALYS.md` avsnitt 7.2: den enda svenska trappräknaren är en artikel från 2012 med en app uppdaterad 2020, och varken den eller sidan som rankar nämner Boverket.

**Två rättelser mot beställningen, båda verifierade mot författningstexten och bekräftade i granskningen 2026-09-20.** Uppdraget sa BFS 2024:8. Den författningen heter *Boverkets föreskrifter om skydd med hänsyn till hygien, hälsa och miljö samt hushållning med vatten och avfall* och innehåller inte ordet trappa. Trappor ligger i **BFS 2024:9**, Boverkets föreskrifter om säkerhet vid användning av byggnader, 2 kap. Och den föreskriften **anger inga stegmått alls**: 2 kap. 5 § lyder i sin helhet "Trappor och ramper ska vara utformade så att personer kan förflytta sig säkert", och de nya föreskrifterna har inga allmänna råd. Det går alltså inte att skriva "uppfyller Boverket" om en steghöjd, och verktyget gör det inte heller. Det säger att myndigheten inte anger stegmått, att måtten det mäter mot är branschens plus borttagna allmänna råd ur gamla BBR, och att de två är olika saker.

**Formuleringen är alltid "inga stegmått", aldrig "inga mått".** Föreskriften innehåller åtta bindande tal, och verktyget citerar tre av dem i samma tabell. Testskriptet vaktar både modulen och rutten mot den bredare formuleringen.

**De borttagna råden står som Källa, och en egen rad märkt Antagande bär beslutet.** Talen 0,25 m, 0,30 m och 1,3 m står ordagrant i BBR, och kolumnen svarar på var ett värde kommer ifrån, så de är Källa. Men beslutet att låta räknaren säga "Måttet håller inte" mot ett upphävt allmänt råd är vårt, och det står sedan 2026-09-20 som raden "Att vi mäter mot de gamla råden". Alternativet vore att inte pröva stegdjupet alls. En rad till, "Standarden Boverket pekar på", namnger SIS/TS 59:2025 och säger att den ligger bakom betalvägg och att vi inte läst den.

Formeln ligger i `src/lib/kalkyl/trappa.ts`:

```ts
export interface TrappaIndata { vaningshojdMm: number; styrs: 'stegdjup' | 'langd'; onskatStegdjupMm: number; tillgangligLangdMm: number; placering: 'inne' | 'ute'; utforande: 'rak' | 'vilplan' }
export const STANDARD = { vaningshojdMm: 2700, styrs: 'stegdjup', onskatStegdjupMm: 300, tillgangligLangdMm: 4400, placering: 'inne', utforande: 'rak' };
export const GRANSER = { vaningshojdMm: [250, 6000], onskatStegdjupMm: [200, 400], tillgangligLangdMm: [500, 20000] };
export function trappformel(steghojdMm, stegdjupMm): number;   // 2h + d
export function lutning(steghojdMm, stegdjupMm): number;       // grader
export function minstaStegdjup(placering): number;             // 250 inne, 300 ute
export function millimeter(n), endecimal(n), meter(mm): string;
export function raknaTrappa(i): TrappaResultat;                // ok | ogiltig
export function tolkaQuery(q): { indata: TrappaIndata; harIndata: boolean };
```

Query: `hojd`, `styrs`, `djup`, `langd`, `plats` och `utforande`. Decimalkomma, mellanslag i talet och ett efterhängande "mm" tolkas. Resultatet bär antalet stigningar, antalet plansteg att bygga, steghöjden exakt och i hela millimeter, stegdjupet, längden i plan, lutningen, trappformelns summa, om summan ligger i trappbranschens bästa spann, de två loppen vid vilplan, om alla mått håller, avvikelserna med källa per rad, beskedet och reglerna.

**Konstanterna står överst med flit**, eftersom guiden `/golv/bygga-trappa/` skriver med exakt samma tal och skribenten ska kunna läsa dem där i stället för att leta upp källorna igen.

**Guiden äger frasen, verktyget äger räkningen.** Första versionens avsnitt om trappformeln delade sjuttioåtta sjugram och sju hela meningar ordagrant med guiden, som är 890 i månaden mot verktygets 50. Två sidor med samma stycken konkurrerar med varandra på samma fras, och den som ska vinna är guiden. Avsnittet är därför kortat till två stycken med egna ord: att myndigheten inte har några stegmått och vad 5 § säger i stället, och att räknaren därför mäter mot branschens tal och mot två borttagna allmänna råd och säger vilket som är vilket i varje svar. Gånglinjen, räcket och den fria höjden ligger kvar i antagandetabellen och i "Därför blev svaret så", och guiden bär resten. Samma regel gäller varje verktyg som bor i en artikel.

**Standardvärdena är guidens räkneexempel.** En våningshöjd på 2 700 mm och ett önskat stegdjup på 300 mm ger sexton steg, 169 mm höga och 292 mm djupa, 4,4 m i plan och 30 graders lutning. Exakt guidens tal, och testskriptet läser dem ur artikeln.

**Räkningen i sex steg.** Antalet stigningar söks i det intervall där steghöjden håller sig i Svenskt Träs 140 till 200 mm. Antalet väljs efter vad som styr trappan: ett önskat stegdjup ger det antal vars formeldjup kommer närmast önskemålet, en given längd i plan ger det antal som får trappformelns summa närmast målet. Steghöjden är våningshöjden delad med antalet och avrundas aldrig, eftersom stegen ska summera till exakt våningshöjden. Stegdjupet kommer ur formeln eller ur längden delad på planstegen och avrundas **nedåt** till hel millimeter, eftersom det är måttet man kapar efter och en trappa som blir en aning kortare får plats där en som blir en aning längre inte gör det; 292,5 blir 292, precis som i guiden. Längden i plan är stegdjupet gånger de plansteg som bär den, alltså ett mindre än antalet stigningar på en rak trappa och två mindre med vilplan, som tar ett stegs plats och 1 300 mm i längdled. Sist prövas måtten.

**Verktyget räknar aldrig fram ett steg som bryter mot ett mått utan att säga det.** Avvikelserna är en egen lista med källa per rad, beskedet i spalten bär dem, och de står överst i "Därför blev svaret så" med varningsfärgad kant. Prövningen gäller steghöjdens spann, stegdjupets undre kant efter placering, trappformelns spann, lutningens spann och antalet steg ute.

**Konstanterna med källa.** Svenskt Trä, byggbeskrivningen Trappor: formeln 600 till 650 mm, steghöjd 140 till 200 mm, normalt plansteg cirka 300 mm, stigningsvinkel 17 till 30 grader, fall ute cirka 1:50. Trappföretaget Stepsta: 620 till 630 mm som bästa summa. BFS 2024:9, hämtad som PDF och läst i sin helhet: inga stegmått (2 kap. 5 §), öppning mellan plansteg högst 100 mm (6 §), fri höjd minst 2,00 m (25 §), räckets underkant till stegnos högst 50 mm och klätterskydd på 0,8 m av höjden (11 §), ledstänger på båda sidor med undantag (12 §). Borttagna allmänna råd ur BBR (BFS 2011:6 t.o.m. 2014:3): stegdjup minst 0,25 m inne mätt i gånglinjen (8:232), minst 0,30 m i gångväg på tomt och fler än två steg (8:91), trapplan minst 1,3 m inom en bostadslägenhet (8:232), räcken 0,9 och 1,1 m (8:2321). Övergången, alltså att äldre bestämmelser fick tillämpas om lovansökan eller anmälan kom in före 1 juli 2026, ur övergångsbestämmelserna till BFS 2024:14 punkt 3. ANTAGANDE: målsumman 630 mm, att vilplanet läggs på mitten med det längre loppet nederst, ledstångens höjd, avrundningen nedåt av stegdjupet och gränserna på fälten.

Markup: H1 "Räkna ut trappans steghöjd, stegdjup och längd på golvet", ingress på tre meningar, `<Faktaruta variant="kortsvar">` på fem korta meningar, sedan formuläret och svaret i två spalter på det linjerade papperet. Resultatspalten bär beskedet, talet och länkarna: beskedet som en rad i H3-stil med en mening under, sedan det stora talet, alltså steghöjden i hela millimeter i `text-siffra` med `<Markering>` och "mm på N steg" på samma baslinje, en rad i `text-liten` med den exakta steghöjden och varför den inte får rundas av, fyra eller fem rader med stegdjup och plansteg, längd i plan, lutning, formelns summa och vilplanets två lopp, en rad som pekar vidare till avsnitten under verktyget, länken "Så räknar vi" och den delbara adressen. Vid standardvärdena är spalten 484 tecken. Direkt under kalkylatorn står H2 "Därför blev svaret så" med avvikelserna först och reglerna under, var och en med sitt slag som etikett (Måttet, Formeln, Boverket, Utförandet), och H2 "Gör inte det här" med de fyra raderna som stycken. Sedan H2 "Vad räknaren mäter mot, och varför det inte är Boverkets mått" i två stycken, H2 "Så räknar vi" med sex steg och antagandetabellen på tjugo rader, H2 "Läs vidare" till `/golv/bygga-trappa/`, `/rakna/kvadratmeter/` och `/rakna/rotavdrag/`, och tre frågor i `<Faq>`.

Formuläret i `src/components/kalkyl/TrappaForm.astro`: våningshöjden som eget fält med hjälpraden om färdigt golv, sedan legenden "Vad styr trappan?" med två radioknappar som var och en har sitt talfält under sig, och sist placering och utförande som radioknappar med en förklarande rad under varje alternativ. **Båda talfälten står alltid framme.** Ett formulär som döljer det ena efter vald radioknapp kräver skript, och sidan får inte ha något; i stället säger en hjälprad att radioknappen avgör vilket fält som läses.

`<title>` "Räkna steghöjd och stegdjup till trappan", 40 tecken. Slugen finns i `MED_FORMULAR`, och `<Kalkylator namn="trappa" />` är inbäddad i `/golv/bygga-trappa/` direkt efter måtttabellen, alltså där läsaren just sett vad formeln ger.

Registret: `pelare: ['golv']` och säsong augusti till oktober, med toppen i september. Innetrappan byggs när det är kallt ute och utetrappan innan hösten.

Testskriptet `scripts/test-kalkyl-trappa.mjs` kör tjugotvå fall. Guiden `/golv/bygga-trappa/` läses från disk och parsas i skriptet, så att en ändrad rad i måtttabellen eller ett omskrivet räkneexempel bryter testet i stället för att tyst glida ifrån verktyget: tabellens fem rader prövas mot trappformeln och mot lutningsfunktionen, och räkneexemplets sex tal jämförs mot verktygets svar vid standardvärdena. Utöver dem testas avrundningen nedåt, att antalet steg följer önskat stegdjup åt rätt håll, att steghöjden håller sig i spannet på varje våningshöjd mellan 1,4 och 4 m, längdläget, en för kort längd, vilplanet, en längd som vilplanet äter upp, skillnaden mellan inne och ute, en utetrappa med två steg, att Boverketsraderna bär rätt paragrafer och säger att stegmått saknas, att det gamla rådet presenteras som borttaget, gränserna, de fyra råden, att markeringskravet bär småhusundantaget i 2 kap. 8 § andra stycket, att varken modulen eller rutten skriver "inga mått", att rutten bär raden om att vi själva valt att mäta mot upphävda råd, `tolkaQuery` och talformateringen.
